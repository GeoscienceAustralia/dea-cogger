from streamer import get_indexed_info, get_prefixes, subset_of_s3_keys, check_nci_to_s3_, compare_nci_with_aws
...
def test_compare_nci_with_aws():
    ...
    cfg = yaml.load(DEFAULT_CONFIG)
    with tempfile.TemporaryDirectory() as tmpdir:
        result = compare_nci_with_aws(cfg, product, 2018, 5, bucket, join(tmpdir, output_file))
